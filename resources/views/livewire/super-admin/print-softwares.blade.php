<div class="superadmin-container">
    ...
    <div class="section-wrapper shadow-md rounded-lg p-6 mb-6 border border-gray-200">
        <form wire:submit.prevent="printSoftwares" class="space-y-6">
            <div>
            ...
            <table class="user-table min-w-full divide-y divide-gray-200 text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($printLogs as $log)
                        <tr>
                            <td data-label="Print Code" class="px-4 py-2 text-center">{{ $log->print_code }}</td>
                            <td data-label="Date From" class="px-4 py-2 text-center">{{ $log->date_from ? \Carbon\Carbon::parse($log->date_from)->format('M d, Y') : 'All' }}</td>
                            <td data-label="Date To" class="px-4 py-2 text-center">{{ $log->date_to ? \Carbon\Carbon::parse($log->date_to)->format('M d, Y') : 'All' }}</td>
                            <td data-label="Printed At" class="px-4 py-2 text-center">{{ $log->created_at->format('M d, Y H:i') }}</td>
                            <td data-label="Printed By" class="px-4 py-2 text-center">{{ $log->user->name }}</td>                            
