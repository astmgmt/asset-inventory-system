...
class AdminHistoryAssets extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        return view('livewire.super-admin.admin-history-assets', [
    ...
    public function generatePdf($historyId)
    {
        ...
        return response()->streamDownload(
            fn () => print($pdf->output()),
            "admin-history-{$history->borrow_code}.pdf"
        );
    }
    ...
    public function generateHistoryPdf($historyId)
    {
        ...
        $returnReceivedBy = $history->return_data['returned_by'] ?? 'N/A';
        ...
        $transactionData = [
            ...
            'returned_at' => isset($history->return_data['returned_at']) 
                ? \Carbon\Carbon::parse($history->return_data['returned_at'])
                    ->setTimezone($timezone)
                    ->format('M d, Y H:i') 
                : 'N/A',
        ...
        $pdf = Pdf::loadView('pdf.admin-history-details', [
        ...
        return response()->streamDownload(
            ...
            "Admin-Asset-History-{$history->borrow_code}.pdf"
        );
    }
    ...
    private function sendDeletionEmail($history)
    {
        try {
            ...
            $body = view('emails.admin-history-deleted', [
                'history' => $history,
                'user' => $user
            ])->render();
            ...
            $emailService->send(
                $user->email,
                "Admin History Deleted: {$history->borrow_code}",
                $body,
                [],
                null,
                null,
            ...
            Log::error("Admin deletion email failed: " . $e->getMessage());
        }
    }
